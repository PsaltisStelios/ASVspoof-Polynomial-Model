...
class Model(torch_nn.Module):
    ...
    def _compute_embedding(self, x, datalength):
        ...
        for idx, (fs, fl, fn, trunc_len, m_trans, m_be_pool, m_output) in \
            ...
            # LOGITS CODE PART (2/3)
            logits = tmp_emb.view(batch_size, -1)
            self.logits_list.append(logits.detach().cpu())      
    ...
    def forward(self, x, fileinfo):
        ...
        if self.training:
            ...
            # LOGITS CODE PART (3/3)
            logits = self.logits_list[-1]
            output_file = "logits.txt"
            if self.logits_list: 
                logits = self.logits_list[-1]
                with open(output_file, 'a') as f:  
                    for logit in logits:
                        f.write(f"{logit.item()}\n")
            else:
                print("No logits to write!")
